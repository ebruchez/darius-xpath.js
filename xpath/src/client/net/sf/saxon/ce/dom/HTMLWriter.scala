...
object HTMLWriter {
  ...
  private def addNamespace(element: Element, prefix: String, uri: String) {
   ...
   */
  def setAttributePropsOriginal(element: Element, localName: String, `val`: String) {
   ...
   */
  def setAttributeProps(element: Element, localName: String, `val`: String) {
    if (Configuration.getIeVersion > 0 && Configuration.getIeVersion < 9) {
      if (localName == "style") {
        if (hasStyle(element)) {
          setStyleProperties(element, `val`)
   ...
   */
  def setStyleProperties(element: Element, styleAttribute: String) {
...
class HTMLWriter extends Receiver {
  ...
  def addNode(newNode: JavaScriptObject, nodeType: String) {
    try {
      if (nextSibling != null && level == 0) {
   ...
   * attachment point.
   */
  def setNextSibling(nextSibling: Node) {
