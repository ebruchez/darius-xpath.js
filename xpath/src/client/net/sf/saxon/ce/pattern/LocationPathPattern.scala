...
object LocationPathPattern {
  ...
  private var EMPTY_FILTER_ARRAY: Array[Expression] = new Array[Expression](0)
}
...
class LocationPathPattern extends Pattern {
   ...
   */
  def setUpperPattern(axis: Byte, upper: Pattern) {
   ...
   */
  def addFilter(filter: Expression) {
  ...
  private def makeEquivalentExpression(): Expression = {
    ...
    var step = new AxisExpression(axis, nodeTest)
