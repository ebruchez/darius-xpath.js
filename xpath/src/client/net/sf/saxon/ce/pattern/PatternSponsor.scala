...
class PatternSponsor(@BeanProperty var pattern: Pattern) extends Expression {
   ...
   */
  def getImplementationMethod(): Int = EVALUATE_METHOD
   ...
   * @return true, always
   */
  def hasLoopingSubexpression(child: Expression): Boolean = true
   ...
   *         the expression
   */
  def getDependencies(): Int = pattern.getDependencies
   ...
   */
  def iterateSubExpressions(): Iterator[Expression] = pattern.iterateSubExpressions()
   ...
   */
  def getContainer(): Container = pattern
   ...
   */
  def evaluateAsString(context: XPathContext): CharSequence = evaluateItem(context).getStringValue
