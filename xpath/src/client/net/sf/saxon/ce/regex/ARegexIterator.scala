...
class ARegexIterator(var theString: UnicodeString, var regex: UnicodeString, var matcher: REMatcher)
  ...
  private var next: UnicodeString = null

  private var prevEnd: Int = 0

  private var nestingTable: HashMap[Integer, Integer] = null
   ...
   * does not match
   */
  def isMatching(): Boolean = next == null && prevEnd >= 0
  ...
  def getRegexGroupIterator(): SequenceIterator = {
    ...
    if (c == 0) {
      ...
      val groups = Array.ofDim[StringValue](c)
      var i = 1
