...
object Division {
      ...
      if (normA(j) == firstDivisorDigit) {
        ...
        if (guessDigit != 0) {
          ...
          do {
             ...
             */
            rightHand = (rem.toLong << 32) + (normA(j - 2) & 0xffffffffL)
            val longR: Long = (rem & 0xffffffffL) + (firstDivisorDigit & 0xffffffffL)
            /*
             ...
             */
            if (java.lang.Integer.numberOfLeadingZeros((longR >>> 32).toInt) < 32) {
              rOverflowed = true
            } else {
              rem = longR.toInt
            }
   ...
   */
  def divideAndRemainderByInteger(bi: BigInteger, divisor: Int, divisorSign: Int): Array[BigInteger] = {
  ...
  def divideArrayByInt(dest: Array[Int],
                       ...
                       divisor: Int): Int = {
    var rem:Long = 0
    val bLong:Long = divisor & 0xffffffffL
    var i = srcLength - 1
    while (i >= 0) {
  ...
  def gcdBinary(val1: BigInteger, val2: BigInteger): BigInteger = {
   ...
   def loop: Unit = {// INV: op2 >= op1 && both are odd unless op1 = 0
     do {
   ...
   */
  def gcdBinary(val1: Long, val2: Long): Long = {
    var op1= val1
    var op2= val2

    val lsb1 = java.lang.Long.numberOfTrailingZeros(op1)
    val lsb2 = java.lang.Long.numberOfTrailingZeros(op2)
    val pow2Count = Math.min(lsb1, lsb2)
    if (lsb1 != 0) {
      op1 >>>= lsb1
    }
    ...
    do {
      if (op1 >= op2) {
        op1 -= op2
        op1 >>>= java.lang.Long.numberOfTrailingZeros(op1)
      } else {
        op2 -= op1
  ...
  def oddModPow(base: BigInteger, exponent: BigInteger, modulus: BigInteger): BigInteger = {
    ...
    res = if (modulus.numberLength == 1) squareAndMultiply(x2, a2, exponent, modulus, n2)
    else slidingWindow(x2,
      a2, exponent, modulus, n2)
  ...
  def remainderArrayByInt(src: Array[Int], srcLength: Int, divisor: Int): Int = {
    ...
    var i = srcLength - 1
    while (i >= 0) {
      val temp: Long = (result << 32) + (src(i) & 0xffffffffL)
  ...
  private def calcN(a: BigInteger): Int = {
    val m0: Long = a.digits(0) & 0xFFFFFFFFL
    var n2: Long = 1L
