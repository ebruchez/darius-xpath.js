...
import org.orbeon.darius.xpath.expr.z.IntPredicate
import org.orbeon.darius.xpath.regex.Operation._
...
object Operation {
  ...
  class OpEndProgram extends Operation {
    ...
    override def nextAction(idx: Int): Int = ACTION_RETURN
  ...
  class OpBranch extends Operation {
    ...
    def exec(matcher: REMatcher, _node: Int, idx: Int): Int = {
      var node = _node
      var idxNew: Int = 0
      do {
  ...
  class OpStar extends Operation {
    ...
    override def nextAction(idx: Int): Int = {
      if (idx == -1) {
  ...
  class OpConfidentStar extends Operation {
    ...
    def exec(matcher: REMatcher, node: Int, _idx: Int): Int = {
      var idx = _idx
      if (matcher.beenHereBefore(idx, node)) {
        return -1
      }
    ...
    override def nextAction(idx: Int): Int = ACTION_ADVANCE_TO_NEXT
  ...
  class OpContinue extends Operation {
    ...
    override def nextAction(idx: Int): Int = ACTION_ADVANCE_TO_FOLLOWING
